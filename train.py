...
if not os.path.exists(f"{os.path.join(args.dir,'bpe')}.model"):
    ...
    spm.SentencePieceTrainer.train(input=args.train, model_prefix=os.path.join(args.dir,'bpe'), train_extremely_large_corpus=True, vocab_size=args.vocab_size, model_type="bpe", character_coverage = 1, user_defined_symbols=['\\n', "\\b"], bos_id=-1,eos_id=-1, pad_id=-1)
